...
public:
    ...
    bool insertarVertices(T dato){
      Vertice<T>* ver=new Vertice<T>(dato);
      this->vertices.push_back(*ver);
       list<Arista<T> > aux;
      this->conexiones.push_back(aux);
      return true;
    }
    ...
    bool insertarAristas(T ver_org, T ver_des, int peso){
      int indice=obtenerIndice(ver_org);
      list<Arista<T>>* conex=obtenerConexionesVertice(indice);
      Vertice<T>* verDes = new Vertice<T>(ver_des);
    ...
    void imprimiAristas()
    {
        ...
        for(; itVertices!=vertices.end(); itVertices++)
        {
            ...
            {
                std::cout<< "ver_org: " <<*itVertices->getDato()<<" --  ver_des: "<<*itConexiones->getSucesor()->getDato()<<endl;
            }
            cout<<endl;
